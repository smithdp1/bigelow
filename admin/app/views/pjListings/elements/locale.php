	...
	if ($cnt > 1)
	{
		...
		foreach ($tpl['locale_arr'] as $k => $locale)
		{
			...
			<li class="<?php echo $k > 0 ? ($k + 1 != $cnt ? NULL : 'property-buttonset-last') : 'property-buttonset-first'; ?>">
				<a class="<?php echo $controller->getLocaleId() != $locale['id'] ? NULL : 'property-menu-locale-focus'; ?>" href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?controller=pjListings&amp;action=pjActionSetLocale&amp;locale=<?php echo $locale['id']; ?><?php echo isset($_GET['iframe']) ? '&amp;iframe' : NULL; ?>">
