...
if (isset($tpl['status']))
{
	...
	if (isset($tpl['arr']))
	{
		if (is_array($tpl['arr']))
		{
			...
			{
				?>
				<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOptions&amp;action=pjActionUpdate" method="post" class="form pj-form tblError" id="frmOptions">
				...
				foreach ($tpl['period_arr'] as $period)
				{
					?>
					<tr>
						<td><span><input type="text" name="days[<?php echo $period['id']; ?>]" class="pj-form-field align_right w50 digits" value="<?php echo $period['days']; ?>" /> <?php __('lblDays'); ?></span></td>
						<td>
							<span class="pj-form-field-custom pj-form-field-custom-before">
				...
				<table id="tblPeriodClone" style="display: none">
					<tbody>
						<tr>
							<td><span><input type="text" name="days[{INDEX}]" class="pj-form-field align_right w50 digits" /> <?php __('lblDays'); ?></span></td>
							<td>
								<span class="pj-form-field-custom pj-form-field-custom-before">
									<span class="pj-form-field-before"><abbr class="pj-form-field-icon-text"><?php echo pjUtil::formatCurrencySign(NULL, $tpl['option_arr']['o_currency'], ""); ?></abbr></span>
