...
class pjListings extends pjFront
{
	...
	public function pjActionIndex()
	{
		...
		switch ($this->option_arr['o_sort_by'])
		{
			...
			case 'created_desc':
				...
				break;
			case 'random':
				$pjListingModel->orderBy(sprintf("FIELD(is_featured, 'T', 'F') ASC, RAND(%u)", $this->defaultTime), false);
				break;
			case 'popularity':
			default:
	...
	public function pjActionSearch()
	{
		if (isset($_GET['listing_search']))
		{
			...
			{
				$refid = $pjListingModel->escapeStr($_GET['refid']);
			...
			if (isset($_GET['bathrooms_from']) && (float) $_GET['bathrooms_from'] > 0)
			{
				$pjListingModel->where('t1.listing_bathrooms >=', $_GET['bathrooms_from']);
			}
			if (isset($_GET['bathrooms_to']) && (float) $_GET['bathrooms_to'] > 0)
			{
			...
			{
				$address_state = $pjListingModel->escapeStr($_GET['address_state']);
				$address_state = str_replace(array('%', '_'), array('\%', '\_'), trim($address_state));
				$pjListingModel->where("t1.address_state LIKE '%". $address_state ."%'");
			}
			if (isset($_GET['address_city']) && !empty($_GET['address_city']))
			{
				$address_city = $pjListingModel->escapeStr($_GET['address_city']);
				$address_city = str_replace(array('%', '_'), array('\%', '\_'), trim($address_city));
				$pjListingModel->where("t1.address_city LIKE '%". $address_city ."%'");
			}
			...
			if (isset($_GET['data']) && !empty($_GET['data']) && strpos($_GET['data'], ",") !== false && pjObject::getPlugin('pjDrawSearch') !== NULL)
			{
				...
				preg_match_all('/(\-?\d+(?:\.\d+)?),\s*(\-?\d+(?:\.\d+)?)/', $_GET['data'], $m);
				$data_str = preg_replace('/\((\-?\d+(?:\.\d+)?),\s*(\-?\d+(?:\.\d+)?)\)/', '\2 \1', $_GET['data']);
				$data_str .= sprintf(",%s %s", $m[2][0], $m[1][0]);
				#$pjListingModel->debug(1);
				$pjListingModel->where('t1.lat IS NOT NULL');
				$pjListingModel->where('t1.lng IS NOT NULL');
				$pjListingModel->where("GISWithin(Point(t1.lng, t1.lat), GeomFromText('Polygon(($data_str))'))");
			}
		...
		if (isset($_GET['address_state']) && !empty($_GET['address_state']))
		{
			$this->pjActionGetCitiesBy('address_state', $_GET['address_state']);
		} elseif (isset($_GET['country_id']) && (int) $_GET['country_id'] > 0) {
			$this->pjActionGetCitiesBy('country_id', $_GET['country_id']);
		}
	...
	private static function pjActionConfirmSend($option_arr, $booking_arr, $listing_arr, $type)
	{
		...
		$tokens = pjAppController::getTokens($booking_arr, $listing_arr, $option_arr);
	...
	public function pjActionGetCities()
	{
		$this->setAjax(true);
		
		if ($this->isXHR())
		{
			$this->pjActionGetCitiesBy($_GET['fieldName'], @$_GET['fieldValue']);
		} else {
			exit;
		}
	}
	
	private function pjActionGetCitiesBy($columnName, $columnValue=NULL)
	{
		$columnName = pjListingModel::escapeString($columnName);
		
		$this->set('city_arr', pjListingModel::factory()
			->select('t1.address_city')
			->where('t1.'.$columnName, $columnValue)
			->groupBy('t1.address_city')
			->orderBy('t1.address_city ASC')
			->findAll()
			->getDataPair(null, 'address_city')
		);
	}
	...
	public function pjActionChangeDt()
	{
		$this->setAjax(true);
	
		if ($this->isXHR())
		{
			if (isset($_POST['date_from'], $_POST['date_to'], $_POST['listing_id'])
				&& !empty($_POST['date_from'])
				&& !empty($_POST['date_to'])
				&& (int) $_POST['listing_id'] > 0)
			{
				$pjListingModel = pjListingModel::factory();
				$arr = $pjListingModel->find($_POST['listing_id'])->getData();
				
				if (empty($arr))
				{
					pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 101, 'text' => 'Listing not found.'));
				}
				...
				if (!preg_match($this->isoDatePattern, $date_from) || !preg_match($this->isoDatePattern, $date_to))
				{
					# Invalid dates
					pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 105, 'text' => $front_bs[5]));
				}
				
				$p_from = strtotime($date_from);
				$p_to = strtotime($date_to);
			
				if ($p_from > $p_to)
				{
					# Invalid period
					pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 106, 'text' => $front_bs[6]));
				}
				...
				{
					# Past period
					pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 108, 'text' => $front_bs[8]));
				}
				
				$numOfDays = ($p_to - $p_from) / 86400;
					
				if ((int) $arr['o_price_based_on'] === 1)
				{
					$numOfDays += 1;
				}

				if ($numOfDays < (int) $arr['o_min_booking_lenght'])
				{
					pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 109, 'text' => sprintf($front_bs[9], $arr['o_min_booking_lenght'], @$listing_b_dn[$arr['o_price_based_on']])));
				}
				
				if ($arr['o_max_booking_lenght'] != '' && $numOfDays > (int) $arr['o_max_booking_lenght'])
				{
					pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 110, 'text' => sprintf($front_bs[10], $arr['o_max_booking_lenght'], @$listing_b_dn[$arr['o_price_based_on']])));
				}
			}
