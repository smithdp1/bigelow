...
class pjAdminTypes extends pjAdmin
{
	...
	public function pjActionGetType()
	{
		...
		if ($this->isXHR())
		{
			$pjTypeModel = pjTypeModel::factory()->join('pjMultiLang', "t2.foreign_id = t1.id AND t2.model = 'pjType' AND t2.locale = '".$this->getLocaleId()."' AND t2.field = 'name'", 'left outer');
			
			if (isset($_GET['q']) && !empty($_GET['q']))
			{
				$q = $pjTypeModel->escapeStr($_GET['q']);
	...
	public function pjActionStatusType()
	{
		$this->setAjax(true);
		...
		if ($this->isXHR())
		{
			...
			{
				pjTypeModel::factory()->whereIn('id', $_POST['record'])->modifyAll(array(
