...
VRL.Utils = {
	...
	changeDate: function () {
		...
		JABB.Ajax.postJSON(this.form.folder.value + "index.php?controller=pjListings&action=pjActionChangeDt", function (data) {
			var el = JABB.Utils.getElementsByClass("selector-err", this.form, 'p')[0];
			if (el) {
				if (data.status == "ERR") {
					el.innerHTML = ['<span class="property-status-error">', data.text, '</span>'].join("");
					el.style.display = "";
				} else {
	...
	changeCountry: function (el, folder, stateClass) {
		...
		var url2 = [folder, "index.php?controller=pjListings&action=pjActionGetCities&fieldName=country_id&fieldValue=", el.options[el.selectedIndex].value, "&cityClass=", stateClass].join("");
		JABB.Ajax.sendRequest(url2, function (xhr) {
			var c = document.getElementById("vrlCityBox");
			if (c) {
				c.innerHTML = xhr.responseText; 
			}
		});
	},
	changeState: function (el, folder, cityClass) {
		var url = [folder, "index.php?controller=pjListings&action=pjActionGetCities&fieldName=address_state&fieldValue=", el.options[el.selectedIndex].value, "&cityClass=", cityClass].join("");
		JABB.Ajax.sendRequest(url, function (xhr) {
			var c = document.getElementById("vrlCityBox");
			if (c) {
				c.innerHTML = xhr.responseText; 
